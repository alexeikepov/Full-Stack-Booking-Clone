...
export default function FindFriends() {
  ...
  const searchUsersHandler = async (query: string) => {
    ...
    try {
      ...
      alert("Failed to search users: " + (error as any)?.message);
    } finally {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <div className="mb-6">
        ...
        <p className="text-gray-600 mt-2">Search for users and send friend requests</p>
      </div>
      ...
      {searchQuery && searchResults.length === 0 && !searchLoading && (
        <Card>
          <CardContent className="text-center py-8">
            ...
            <p className="text-sm text-gray-400 mt-1">Try searching with a different name or email</p>
          </CardContent>
        </Card>
      )}
    </div>
