...
`;
  ...
  const rulesState = {
    ...
    notEmail:
      password.length > 0 && !password.toLowerCase().includes(emailName),
        ...
        {showBanner && (
          <div className="mx-auto mb-6 w-full max-w-[400px] rounded-[10px] border border-[#e6e6e6] bg-[#f7f7f7] p-3">
            <div className="flex items-start gap-3 text-[13.5px] leading-6 text-[#2d2d2d]">
              ...
              <span>
                We’ve completed the signup form using your account information
                as you’re logged in to your Booking.com account.
              </span>
              <button
                aria-label="close"
        ...
        <div className="mx-auto w-full max-w-[400px] stage">
          ...
          {step === "password" && (
            <section className={isLeaving ? "panel leaving" : "panel entering"}>
              ...
              <input
                className={`input mt-1 ${
                  confirmTouched && !rulesState.match ? "error" : ""
                }`}
                type="password"
                value={confirm}
              ...
              {showChecklist && (
                <ul className="help-list">
                  <li className={rulesState.length ? "ok" : "bad"}>
                    At least 10 characters
                  </li>
                  <li className={rulesState.upper ? "ok" : "bad"}>
                    Contains an uppercase letter
                  </li>
                  <li className={rulesState.lower ? "ok" : "bad"}>
                    Contains a lowercase letter
                  </li>
                  <li className={rulesState.number ? "ok" : "bad"}>
                    Contains a number
                  </li>
                  <li className={rulesState.notEmail ? "ok" : "bad"}>
                    Does not contain your email name
                  </li>
                  <li className={rulesState.match ? "ok" : "bad"}>
                    Passwords match
                  </li>
                </ul>
              )}
