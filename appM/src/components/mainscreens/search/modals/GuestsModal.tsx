...
const GuestsModal = ({
  ...
  // Helper function to calculate minimum rooms needed based on total people
  const calculateMinRooms = (adults: number, children: number) => {
    const totalPeople = adults + children;
    return Math.max(1, Math.ceil(totalPeople / 4));
  };
  ...
  const handleRoomsChange = (increment: boolean) => {
    const minRooms = calculateMinRooms(adults, children);
    const newValue = increment ? rooms + 1 : Math.max(minRooms, rooms - 1);
    setRooms(newValue);
  };
  ...
  const handleAdultsChange = (increment: boolean) => {
    ...
    // Automatically adjust rooms based on total people
    const minRooms = calculateMinRooms(newValue, children);
    setRooms(minRooms);
  };
  ...
  const handleChildrenChange = (increment: boolean) => {
    ...
    // Automatically adjust rooms based on total people
    const minRooms = calculateMinRooms(adults, newValue);
    setRooms(minRooms);
  };
