...
import {
  ...
  SavedItem,
} from "../../../components/mainscreens/saved/index";
...
export default function SavedScreen() {
  ...
  const handleShareList = async () => {
    ...
    try {
      // Find the properties in this list (for now, only the default list is supported)
      const items = savedProperties;
      let message = `Check out my saved list: ${currentList.title}\n`;
      if (items.length > 0) {
        message += items
          .map(
            (item, idx) =>
              `${idx + 1}. ${item.title || "Property"}${
                item.location ? " - " + item.location : ""
              }`,
          )
          .join("\n");
      } else {
        message += "No properties in this list yet.";
      }
      await Share.share({ message });
    } catch (error: any) {
      Alert.alert("Share failed", error?.message || "Could not share list.");
    }
    setManageListModalVisible(false);
  };
